...
MyButton select(selectPin);
MyButton prev(prevPin);
MyButton next(nextPin);
...
const uint8_t mui_hrule(mui_t *ui, uint8_t msg) {
...
const uint8_t mui_yes(mui_t *ui, uint8_t msg) {
  if (msg == MUIF_MSG_DRAW) {
    mui.leaveForm();
    initStatus = 0;
    is_redraw = 0;
...
const uint8_t mui_no(mui_t *ui, uint8_t msg) {
  if (msg == MUIF_MSG_DRAW) {
    mui.leaveForm();
    is_redraw = 1;
    iconTimer.start();
...
void setup() {
  ...
  //Select, Next, Prev, Up, Down, Home/Cancel
  u8g2.begin();
